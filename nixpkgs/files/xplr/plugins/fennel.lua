package.preload["fennel.repl"] = package.preload["fennel.repl"]
  or function(...)
    ...
    local function splice_save_locals(env, lua_source)
      ...
      if
        (1 < #spliced_source)
        and (spliced_source[#spliced_source]):match("^ *return .*$")
      then
      ...
      local function add_partials(input, tbl, prefix, method_3f)
        for k in utils.allpairs(tbl) do
          ...
          if
            (#matches < 2000)
            and (type(k0) == "string")
            and (input == k0:sub(0, #input))
            and (not method_3f or ("function" == type(tbl[k0])))
          then
    ...
    local function command_docs()
      ...
      do
        ...
        for name, f in pairs(commands) do
          local val_16_auto = ("  ,%s - %s"):format(
            name,
            ((compiler.metadata):get(f, "fnl/docstring") or "undocumented")
          )
    ...
    (compiler.metadata):set(
      commands.help,
      "fnl/docstring",
      "Show this message."
    )
    local function reload(module_name, env, on_values, on_error)
      local _534_, _535_ = pcall(
        specials["load-code"]("return require(...)", env),
        module_name
      )
      ...
      local function _547_()
        return on_values(
          completer(
            env,
            scope,
            string.char(unpack(chars)):gsub(",complete +", ""):sub(1, -2)
          )
        )
      end
    ...
    local function apropos_show_docs(on_values, pattern)
      for _, path in ipairs(apropos(pattern)) do
        ...
        if
          ("function" == type(tgt))
          and (compiler.metadata):get(tgt, "fnl/docstring")
        then
    ...
    local function resolve(identifier, _567_, scope)
      ...
      local code = compiler["compile-string"](
        tostring(identifier),
        { scope = scope }
      )
    ...
    commands.find = function(env, read, on_values, on_error, scope)
      local function _570_(_241)
        ...
        if
          (_G.type(_571_) == "table")
          and (nil ~= (_571_).short_src)
          and (nil ~= (_571_).linedefined)
          and ((_571_).what == "Lua")
          and (nil ~= (_571_).source)
        then
    ...
    commands.doc = function(env, read, on_values, on_error, scope)
      local function _581_(_241)
        ...
        local target = (
            scope.specials[name]
            or scope.macros[name]
            or resolve(name, env, scope)
          )
    ...
    local function run_command_loop(
      input,
      read,
      loop,
      env,
    ...
    local function repl(options)
      ...
      local env = specials["wrap-env"](
        (options.env or (rawget(_G, "_ENV") or _G))
      )
      local save_locals_3f = (
          (options.saveLocals ~= false)
          and env.debug
          and env.debug.getlocal
        )
...
package.preload["fennel.specials"] = package.preload["fennel.specials"]
  or function(...)
    local utils = require("fennel.utils")
    local view = require("fennel.view")
    local parser = require("fennel.parser")
    ...
    local function wrap_env(env)
      ...
      return setmetatable(
        {},
        { __index = _345_, __newindex = _347_, __pairs = _349_ }
      )
    end
    ...
    local function doc_2a(tgt, name)
      if not tgt then
        ...
        local docstring = (
          ((compiler.metadata):get(tgt, "fnl/docstring") or "#<undocumented>")
        ):gsub("\n$", ""):gsub("\n", "\n  ")
        local mt = getmetatable(tgt)
        if
          (type(tgt) == "function")
          or ((type(mt) == "table") and (type(mt.__call) == "function"))
        then
          local arglist = table.concat(
            (
                (compiler.metadata):get(tgt, "fnl/arglist")
    ...
    SPECIALS["do"] =
      function(ast, scope, parent, opts, _3fstart, _3fchunk, _3fsub_scope, _3fpre_syms)
        local start = (_3fstart or 2)
          ...
          if len < start then
            ...
            for i = start, len do
              local subopts = {
                ...
                tail = (((i == len) and outer_tail) or nil),
                target = (((i == len) and outer_target) or nil),
              }
              local _ = utils["propagate-options"](opts, subopts)
              local subexprs = compiler.compile1(
                ast[i],
                sub_scope,
                chunk,
                subopts
        ...
        elseif opts.nval then
          ...
          for i = 1, opts.nval do
            local s = (
                (_3fpre_syms and (_3fpre_syms)[i]) or compiler.gensym(scope)
              )
            do
            end
            (syms)[i] = s
          ...
          compiler.emit(
            parent,
            string.format("local function %s(%s)", fname, fargs),
            ast
          )
          return compile_body(
            nil,
    ...
    local function set_fn_metadata(arg_list, docstring, parent, fn_name)
      if utils.root.options.useMetadata then
        ...
        local meta_str = ('require("%s").metadata'):format(
          (utils.root.options.moduleName or "fennel")
        )
    ...
    local function compile_named_fn(
      ast,
      f_scope,
      f_chunk,
      parent,
      ...
      for i = (index + 1), #ast do
        compiler.compile1(
          ...
          f_scope,
          f_chunk,
          { nval = (((i ~= #ast) and 0) or nil), tail = (i == #ast) }
        )
      end
      local _379_
      if local_3f then
      ...
      compiler.emit(
        parent,
        string.format(_379_, fn_name, table.concat(arg_name_list, ", ")),
        ast
      )
    ...
    local function compile_anonymous_fn(
      ast,
      f_scope,
      f_chunk,
      parent,
    ...
    SPECIALS.fn = function(ast, scope, parent)
      ...
      local arg_list = compiler.assert(
        utils["table?"](ast[index]),
        "expected parameters table",
        ast
      )
      compiler.assert(
        (not multi or not multi["multi-sym-method-call"]),
        ("unexpected multi symbol " .. tostring(fn_name)),
        fn_sym
      )
      local function get_arg_name(arg)
        ...
        elseif
          utils["sym?"](arg)
          and (tostring(arg) ~= "nil")
          and not utils["multi-sym?"](tostring(arg))
        then
        ...
        elseif utils["table?"](arg) then
          ...
          compiler.destructure(
            ...
            f_scope,
            f_chunk,
            { declaration = true, nomulti = true, symtype = "arg" }
          )
          return declared
        else
          return compiler.assert(
            false,
            ("expected symbol for function parameter: %s"):format(tostring(arg)),
            ast[index]
          )
        end
      end
    ...
    local function dot(ast, scope, parent)
      ...
      if len == 2 then
        ...
        for i = 3, len do
          local index = ast[i]
          if
            (type(index) == "string") and utils["valid-lua-identifier?"](index)
          then
            table.insert(indices, ("." .. index))
          else
            local _let_394_ = compiler.compile1(
              index,
              scope,
              parent,
              { nval = 1 }
    ...
    SPECIALS.var = function(ast, scope, parent)
      ...
      compiler.destructure(
        ...
        scope,
        parent,
        { declaration = true, isvar = true, nomulti = true, symtype = "var" }
      )
      return nil
    end
    ...
    SPECIALS.let = function(ast, scope, parent, opts)
      ...
      local pre_syms = {}
      compiler.assert(
        (utils["table?"](bindings) and not kv_3f(bindings)),
        "expected binding sequence",
        bindings
      )
      compiler.assert(
        ((#bindings % 2) == 0),
    ...
    SPECIALS.tset = function(ast, scope, parent)
      compiler.assert(
        (#ast > 3),
        "expected table, key, and value arguments",
        ast
      )
      ...
      local value =
        (
          compiler.compile1(ast[#ast], scope, parent, { nval = 1 })
        )[1]
      ...
      return compiler.emit(
        parent,
        fmtstr:format(rootstr, table.concat(keys, "]["), tostring(value)),
        ast
      )
    end
    doc_special(
    ...
    local function if_2a(ast, scope, parent, opts)
      ...
      local branches = {}
      local wrapper, inner_tail, inner_target, target_exprs = calculate_target(
        scope,
        opts
      )
      local body_opts = {
      ...
      local function compile_body(i)
        ...
        compiler["keep-side-effects"](
          compiler.compile1(ast[i], cscope, chunk, body_opts),
          chunk,
          nil,
          ast[i]
        )
      ...
      if wrapper == "iife" then
        ...
        compiler.emit(
          parent,
          ("local function %s(%s)"):format(tostring(s), iifeargs),
          ast
        )
    ...
    local function compile_until(condition, scope, chunk)
      if condition then
        local _let_414_ = compiler.compile1(
          condition,
          scope,
          chunk,
          { nval = 1 }
        ...
        return compiler.emit(
          chunk,
          ("if %s then break end"):format(tostring(condition_lua)),
    ...
    SPECIALS.each = function(ast, scope, parent)
      compiler.assert((#ast >= 3), "expected body expression", ast[1])
      local binding = compiler.assert(
        utils["table?"](ast[2]),
        "expected binding table",
        ast
      )
      local _ = compiler.assert(
        (2 <= #binding),
      ...
      local function destructure_binding(v)
        compiler.assert(
          ("string" ~= type(v)),
          ("unexpected iterator clause " .. tostring(v)),
          binding
        )
        if utils["sym?"](v) then
      ...
      compiler.emit(
        parent,
        ("for %s in %s do"):format(
          table.concat(bind_vars, ", "),
          table.concat(val_names, ", ")
        ),
        ast
      )
    ...
    local function for_2a(ast, scope, parent)
      ...
      local chunk = {}
      compiler.assert(
        utils["sym?"](binding_sym),
        ("unable to bind %s %s"):format(
          type(binding_sym),
          tostring(binding_sym)
        ),
        ast[2]
      )
      compiler.assert((#ast >= 3), "expected body expression", ast[1])
      ...
      for i = 1, math.min(#ranges, 3) do
        range_args[i] = tostring(
          (compiler.compile1(ranges[i], scope, parent, { nval = 1 }))[1]
        )
      end
      compiler.emit(
        parent,
        ("for %s = %s do"):format(
          compiler["declare-local"](binding_sym, {}, sub_scope, ast),
          table.concat(range_args, ", ")
        ),
        ast
      )
      compile_until(until_condition, sub_scope, chunk)
    ...
    local function nonnative_method_call(ast, scope, parent, target, args)
      local method_string = tostring(
        (compiler.compile1(ast[3], scope, parent, { nval = 1 }))[1]
      )
    ...
    local function double_eval_protected_method_call(
      ast,
      scope,
      parent,
      target,
      ...
      return utils.expr(
        string.format(call, tostring(target), table.concat(args, ", ")),
        "statement"
      )
    end
    local function method_call(ast, scope, parent)
      ...
      if
        (type(ast[3]) == "string") and utils["valid-lua-identifier?"](ast[3])
      then
      ...
      elseif target.type == "sym" then
        ...
        return double_eval_protected_method_call(
          ast,
          scope,
          parent,
          target,
    ...
    doc_special(
      "comment",
      { "..." },
      "Comment which will be emitted in Lua output.",
      true
    )
    ...
    SPECIALS.hashfn = function(ast, scope, parent)
      ...
      compiler.emit(
        parent,
        string.format("local function %s(%s)", name, arg_str),
        ast
      )
      compiler.emit(parent, f_chunk, ast)
      compiler.emit(parent, "end", ast)
    ...
    local function arithmetic_special(
      name,
      zero_arity,
      unary_prefix,
      ast,
    ...
    local function define_arithmetic_special(
      name,
      zero_arity,
      unary_prefix,
      _3flua_name
    ...
    doc_special(
      "band",
      { "x1", "x2", "..." },
      "Bitwise AND of any number of arguments.\nOnly works in Lua 5.3+ or LuaJIT with the --use-bit-lib flag."
    )
    doc_special(
      "bor",
      { "x1", "x2", "..." },
      "Bitwise OR of any number of arguments.\nOnly works in Lua 5.3+ or LuaJIT with the --use-bit-lib flag."
    )
    doc_special(
      "bxor",
      { "x1", "x2", "..." },
      "Bitwise XOR of any number of arguments.\nOnly works in Lua 5.3+ or LuaJIT with the --use-bit-lib flag."
    )
    doc_special(
      "..",
      { "a", "b", "..." },
    ...
    local function double_eval_protected_comparator(
      op,
      chain_op,
      ast,
      scope,
      ...
      for i = 2, #ast do
        ...
        table.insert(
          vals,
          tostring((compiler.compile1(ast[i], scope, parent, { nval = 1 }))[1])
        )
      end
      for i = 1, (#arglist - 1) do
        table.insert(
          comparisons,
          string.format("(%s %s %s)", arglist[i], op, arglist[(i + 1)])
        )
      end
      return string.format(
        "(function(%s) return %s end)(%s)",
      ...
      do
        ...
        local function opfn(ast, scope, parent)
          ...
          if 3 == #ast then
            ...
            return double_eval_protected_comparator(
              op,
              _3fchain_op,
              ast,
              scope,
    ...
    doc_special(
      "bnot",
      { "x" },
      "Bitwise negation; only works in Lua 5.3+ or LuaJIT with the --use-bit-lib flag."
    )
    ...
    local function make_compiler_env(ast, scope, parent, _3fopts)
      ...
      do
        ...
        if
          (_G.type(_461_) == "table") and ((_461_)["compiler-env"] == "strict")
        then
        ...
        elseif
          (_G.type(_461_) == "table") and (nil ~= (_461_)["compiler-env"])
        then
      ...
      local function _463_(base)
        return utils.sym(
          compiler.gensym((compiler.scopes.macro or scope), base)
        )
      end
      ...
      return setmetatable(
        env,
        { __index = provided, __newindex = provided, __pairs = combined_mt_pairs }
      )
    end
    ...
    local function search_module(modulename, _3fpathstring)
      ...
      local fullpath = (
          (_3fpathstring or utils["fennel-module"].path) .. pkg_config.pathsep
        )
      local function try_path(path)
        local filename = path:gsub(
          escapepat(pkg_config.pathmark),
          no_dot_module
        )
    ...
    local function fennel_macro_searcher(module_name)
      ...
      local _482_ = search_module(
        module_name,
        utils["fennel-module"]["macro-path"]
      )
      if nil ~= _482_ then
    ...
    local function lua_macro_searcher(module_name)
      ...
      if nil ~= _488_ then
        ...
        do
          ...
          code = close_handlers_8_auto(
            _G.xpcall(_490_, (package.loaded.fennel or debug).traceback)
          )
        end
    ...
    local function metadata_only_fennel(modname)
      if
        (modname == "fennel.macros")
        or (
          package
          and package.loaded
          and ("table" == type(package.loaded[modname]))
          and (package.loaded[modname].metadata == compiler.metadata)
        )
      then
    ...
    local function resolve_module_name(_500_, _scope, _parent, opts)
      ...
      local filename0 = (
          filename or (utils["table?"](second) and second.filename)
        )
    ...
    SPECIALS["require-macros"] = function(ast, scope, parent, _3freal_ast)
      compiler.assert(
        (#ast == 2),
        "Expected one module name argument",
        (_3freal_ast or ast)
      )
    ...
    local function include_path(ast, opts, path, mod, fennel_3f)
      ...
      do
        ...
        src = close_handlers_8_auto(
          _G.xpcall(_507_, (package.loaded.fennel or debug).traceback)
        )
      end
    ...
    SPECIALS.include = function(ast, scope, parent, opts)
      ...
      if (modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34) then
        if opts.fallback then
          ...
          return compiler.assert(
            false,
            "module name must be string literal",
            ast
          )
        end
      else
        ...
        res = (
            (
              utils["member?"](mod, (utils.root.options.skipInclude or {}))
              and utils.expr("nil --[[SKIPPED INCLUDE]]--", "literal")
            )
            or include_circular_fallback(mod, modexpr, opts.fallback, ast)
            or utils.root.scope.includes[mod]
            or _515_()
          )
    ...
    SPECIALS.macros = function(ast, scope, parent)
      ...
      return add_macros(
        eval_compiler_2a(ast[2], scope, parent),
        ast,
        scope,
        parent
      )
    ...
    return {
      ...
      ["load-code"] = load_code,
      ["macro-loaded"] = macro_loaded,
      ["macro-searchers"] = macro_searchers,
...
package.preload["fennel.compiler"] = package.preload["fennel.compiler"]
  or function(...)
    local utils = require("fennel.utils")
    local parser = require("fennel.parser")
    ...
    local function make_scope(_3fparent)
      ...
      return {
        includes = setmetatable({}, { __index = (parent and parent.includes) }),
        macros = setmetatable({}, { __index = (parent and parent.macros) }),
        manglings = setmetatable(
          {},
          { __index = (parent and parent.manglings) }
        ),
        specials = setmetatable({}, { __index = (parent and parent.specials) }),
        symmeta = setmetatable({}, { __index = (parent and parent.symmeta) }),
        unmanglings = setmetatable(
          {},
          { __index = (parent and parent.unmanglings) }
        ),
        gensyms = setmetatable({}, { __index = (parent and parent.gensyms) }),
        autogensyms = setmetatable(
          {},
          { __index = (parent and parent.autogensyms) }
        ),
    ...
    local function assert_compile(condition, msg, ast)
      if not condition then
        ...
        if
          nil
          == utils.hook("assert-compile", condition, msg, ast, utils.root.reset)
        then
          utils.root.reset()
    ...
    local function serialize_string(str)
      ...
      return string.gsub(
        string.gsub(string.format("%q", str), ".", serialize_subst),
        "[\128-\255]",
        _210_
      )
    end
    ...
    local function local_mangling(str, scope, ast, _3ftemp_manglings)
      assert_compile(
        not utils["multi-sym?"](str),
        ("unexpected multi symbol " .. str),
        ast
      )
      local raw
    ...
    local function apply_manglings(scope, new_manglings, ast)
      for raw, mangled in pairs(new_manglings) do
        assert_compile(
          not scope.refedglobals[mangled],
          ("use of global " .. raw .. " is aliased by a local"),
          ast
        )
        do
        end
    ...
    local function next_append()
      utils.root.scope["gensym-append"] = (
          (utils.root.scope["gensym-append"] or 0) + 1
        )
    ...
    local function autogensym(base, scope)
      ...
      if nil ~= _222_ then
        ...
        return table.concat(
          parts,
          ((parts["multi-sym-method-call"] and ":") or ".")
        )
      elseif true then
        local _ = _222_
    ...
    local function check_binding_valid(symbol, scope, ast)
      ...
      assert_compile(
        not (scope.specials[name] or scope.macros[name]),
        ("local %s was overshadowed by a special form or macro"):format(name),
        ast
      )
      return assert_compile(
        not utils["quoted?"](symbol),
    ...
    local function declare_local(symbol, meta, scope, ast, _3ftemp_manglings)
      ...
      local name = tostring(symbol)
      assert_compile(
        not utils["multi-sym?"](name),
        ("unexpected multi symbol " .. name),
        ast
      )
      do
      end
    ...
    local function symbol_to_expression(symbol, scope, _3freference_3f)
      ...
      assert_compile(
        (
            ...
            or local_3f
            or ("_ENV" == parts[1])
            or global_allowed_3f(parts[1])
          ),
        ("unknown identifier in strict mode: " .. tostring(parts[1])),
        symbol
      )
    ...
    local function peephole(chunk)
      ...
      elseif
        (#chunk >= 3)
        and ((chunk[(#chunk - 2)]).leaf == "do")
        and not (chunk[(#chunk - 1)]).leaf
        and (chunk[#chunk].leaf == "end")
      then
    ...
    local function flatten(chunk, options)
      ...
      if options.correlate then
        ...
        if sm then
          sm.short_src = (
              options.filename or make_short_src((options.source or ret))
            )
    ...
    local function make_metadata()
      ...
      return setmetatable(
        {},
        { __index = { get = _245_, set = _247_, setall = _248_ }, __mode = "k" }
      )
    end
    ...
    local function find_macro(ast, scope, multi_sym_parts)
      local function find_in_table(t, i)
        if i <= #multi_sym_parts then
          return find_in_table(
            (utils["table?"](t) and t[multi_sym_parts[i]]),
            (i + 1)
          )
        else
          return t
      ...
      local macro_2a = (
          utils["sym?"](ast[1]) and scope.macros[tostring(ast[1])]
        )
      if not macro_2a and multi_sym_parts then
        local nested_macro = find_in_table(scope.macros, 1)
        assert_compile(
          (
              not scope.macros[multi_sym_parts[1]]
              or (type(nested_macro) == "function")
            ),
    ...
    local function compile_special(ast, scope, parent, opts, special)
      local exprs = (
          special(ast, scope, parent, opts) or utils.expr("nil", "literal")
        )
      local exprs0
    ...
    local function compile_function_call(
      ast,
      scope,
      parent,
      opts,
      ...
      assert_compile(
        (("string" == type(ast[1])) or (fcallee.type ~= "literal")),
        ("cannot call literal value " .. tostring(ast[1])),
        ast
      )
      for i = 2, len do
        local subexprs
      ...
      return handle_compile_opts(
        { utils.expr(call, "statement") },
        parent,
        opts,
        ast
      )
    ...
    local function compile_call(ast, scope, parent, opts, compile1)
      ...
      assert_compile(
        (len > 0),
        "expected a function, macro, or special to call",
        ast
      )
      if special then
      ...
      elseif multi_sym_parts and multi_sym_parts["multi-sym-method-call"] then
        local table_with_method = table.concat(
          { unpack(multi_sym_parts, 1, (#multi_sym_parts - 1)) },
          "."
        )
        local method_to_call = multi_sym_parts[#multi_sym_parts]
        local new_ast = utils.list(
          utils.sym(":", nil, scope),
          utils.sym(table_with_method, nil, scope),
          method_to_call,
          select(2, unpack(ast))
        )
    ...
    local function compile_sym(ast, scope, parent, opts)
      ...
      assert_compile(
        not (multi_sym_parts and multi_sym_parts["multi-sym-method-call"]),
    ...
    local function compile_scalar(ast, _scope, parent, opts)
      ...
      return handle_compile_opts(
        { utils.expr(serialize(ast), "literal") },
        parent,
        opts
      )
    end
    local function compile_table(ast, scope, parent, opts, compile1)
      ...
      return handle_compile_opts(
        { utils.expr(("{" .. table.concat(buffer, ", ") .. "}"), "expression") },
        parent,
        opts,
        ast
      )
    ...
    local function compile1(ast, scope, parent, _3fopts)
      ...
      then
        ...
        return assert_compile(
          false,
          ("could not compile value of type " .. type(ast0)),
          ast0
        )
      end
    end
    local function destructure(to, from, ast, scope, parent, opts)
      ...
      local function getname(symbol, up1)
        local raw = symbol[1]
        assert_compile(
          not (opts0.nomulti and utils["multi-sym?"](raw)),
          ("unexpected multi symbol " .. raw),
          up1
        )
        if declaration then
          ...
          if (#parts == 1) and not forceset then
            assert_compile(
              not (forceglobal and meta),
              string.format("global %s conflicts with local", tostring(symbol)),
              symbol
            )
            assert_compile(
              not (meta and not meta.var),
              ("expected var " .. raw),
              symbol
            )
            assert_compile(
              (meta or not opts0.noundef),
              ("expected local " .. parts[1]),
              symbol
            )
          else
          end
          if forceglobal then
            assert_compile(
              not scope.symmeta[scope.unmanglings[raw]],
      ...
      local function destructure1(left, rightexprs, up1, top_3f)
        ...
        elseif utils["list?"](left) then
          ...
          assert_compile(
            false,
            string.format("unable to bind %s %s", type(left), tostring(left)),
    ...
    local function require_include(ast, scope, parent, opts)
      opts.fallback = function(e)
        utils.warn(
          ("include module not found, falling back to require: %s"):format(
            tostring(e)
          )
        )
        return utils.expr(
          string.format("require(%s)", tostring(e)),
          "statement"
        )
      end
    ...
    local function compile_stream(strm, options)
      ...
      for i = 1, #vals do
        local exprs = compile1(
          vals[i],
          scope,
          chunk,
          { nval = (((i < #vals) and 0) or nil), tail = (i == #vals) }
        )
      ...
      elseif info.what == "C" then
        ...
        if remap and remap[info.currentline] then
          if remap[info.currentline][1] then
            info.short_src =
              sourcemap[("@" .. remap[info.currentline][1])].short_src
          else
    ...
    local function traceback(msg, start)
      ...
      if
        (msg0:find("^Compile error") or msg0:find("^Parse error"))
        and not utils["debug-on?"]("trace")
      then
        return msg0
      else
        local lines = {}
        if
          msg0:find(":%d+: Compile error") or msg0:find(":%d+: Parse error")
        then
    ...
    local function do_quote(form, scope, parent, runtime_3f)
      ...
      if utils["varg?"](form) then
        assert_compile(
          not runtime_3f,
          "quoted ... may only be used at compile time",
          form
        )
        return "_VARARG"
      elseif utils["sym?"](form) then
        ...
        if symstr:find("#$") or symstr:find("#[:.]") then
          return string.format(
            "sym('%s', {filename=%s, line=%s})",
            autogensym(symstr, scope),
            filename,
            (form.line or "nil")
          )
        else
          return string.format(
            "sym('%s', {quoted=true, filename=%s, line=%s})",
            symstr,
            filename,
            (form.line or "nil")
          )
      ...
      elseif
        utils["list?"](form)
        and utils["sym?"](form[1])
        and (tostring(form[1]) == "unquote")
      then
      ...
      elseif utils["list?"](form) then
        ...
        assert_compile(
          not runtime_3f,
          "lists may only be used at compile time",
          form
        )
        return string.format(
          (
...
package.preload["fennel.friend"] = package.preload["fennel.friend"]
  or function(...)
    local utils = require("fennel.utils")
    local suggestions = {
      ...
      ["unexpected arguments"] = { "removing an argument", "checking for typos" },
      ["unexpected iterator clause"] = {
    ...
    local function read_line_from_string(
      matcher,
      target_line,
      _3fcurrent_line,
      _3fbytes
      ...
      if ok and codeline and bytestart and byteend then
        table.insert(
          ...
          (
              ...
              .. string.rep(
                "^",
                math.min((byteend - bytestart), ((bol + #codeline) - bytestart))
              )
            )
        )
      else
    ...
    local function assert_compile(condition, msg, ast, source)
      if not condition then
        ...
        error(
          friendly_msg(
            ("Compile error in %s:%s\n  %s"):format(
              (filename or "unknown"),
              (line or "?"),
              msg
            ),
            utils["ast-source"](ast),
            source
          ),
          0
        )
    ...
    local function parse_error(msg, filename, line, bytestart, source)
      return error(
        friendly_msg(
          ...
          { filename = filename, line = line, bytestart = bytestart },
          source
        ),
        0
      )
...
package.preload["fennel.parser"] = package.preload["fennel.parser"]
  or function(...)
    local utils = require("fennel.utils")
    local friend = require("fennel.friend")
    local unpack = (table.unpack or _G.unpack)
    ...
    local function parser(getbyte, _3ffilename, _3foptions)
      ...
      assert(
        ((nil == _3ffilename) or ("string" == type(_3ffilename))),
      ...
      local function parse_error(msg, byteindex_override)
        ...
        if
          ...
          == utils.hook(
            "parse-error",
            msg,
            (_3ffilename or "unknown"),
            (line or "?"),
            (byteindex_override or byteindex),
            source,
            utils.root.reset
          )
        then
      ...
      local function parse_stream()
        ...
        local function badend()
          ...
          return parse_error(
            string.format(
              "expected closing delimiter%s %s",
              _168_,
          ...
          elseif _3foptions and _3foptions.comments then
            return dispatch(
              utils.comment(
                ...
                { line = (line - 1), filename = _3ffilename }
              )
            )
          else
            return b
        ...
        local function parse_string()
          ...
          local _189_ = (rawget(_G, "loadstring") or load)(
            ("return " .. formatted)
          )
          if nil ~= _189_ then
            local load_fn = _189_
        ...
        local function parse_number(rawstr)
          local number_with_stripped_underscores = (
              not rawstr:find("^_") and rawstr:gsub("_", "")
            )
          if rawstr:match("^%d") then
            dispatch(
              (
                  tonumber(number_with_stripped_underscores)
                  or parse_error(('could not read number "' .. rawstr .. '"'))
                )
            )
            return true
          else
        ...
        local function check_malformed_sym(rawstr)
          ...
          then
            return parse_error(
              ("malformed multisym: " .. rawstr),
              ((byteindex - #rawstr) + 1 + rawstr:find("[%.:][%.:]"))
            )
          ...
          elseif rawstr:match(":.+[%.:]") then
            return parse_error(
              ("method must be last component of multisym: " .. rawstr),
              ((byteindex - #rawstr) + rawstr:find(":.+[%.:]"))
            )
          else
            return rawstr
          end
        ...
        local function parse_sym(b)
          ...
          elseif not parse_number(rawstr) then
            return dispatch(utils.sym(check_malformed_sym(rawstr), {
              byteend = byteindex,
              bytestart = bytestart,
              filename = _3ffilename,
              line = line,
    ...
    return {
      ...
      ["sym-char?"] = sym_char_3f,
    }
  end
local utils
package.preload["fennel.view"] = package.preload["fennel.view"]
  or function(...)
    local type_order = {
      number = 1,
      boolean = 2,
    ...
    local function detect_cycle(t, seen, _3fk)
      if "table" == type(t) then
        ...
        if (nil ~= _27_) and (nil ~= _28_) then
          ...
          return (
              seen[k]
              or detect_cycle(k, seen)
              or seen[v]
              or detect_cycle(v, seen)
              or detect_cycle(t, seen, k)
            )
        else
          return nil
        end
    ...
    local function visible_cycle_3f(t, options)
      return (
          ...
          and detect_cycle(t, {})
          and save_table(t, options.seen)
          and (1 < (options.appearances[t] or 0))
        )
    end
    ...
    local pp = nil
    local function concat_table_lines(
      elements,
      options,
      multiline_3f,
      indent,
      ...
      if
        not options["one-line?"]
        and (
          multiline_3f
          or ((indent + length_2a(oneline)) > options["line-length"])
        )
      then
      ...
      local function validate_utf8(str0, index)
        ...
        do
          ...
          for _, init0 in ipairs(inits) do
            ...
            ret = (
                byte
                and (function(_69_, _70_, _71_)
                  return (_69_ >= _70_) and (_70_ >= _71_)
                end)(init0["max-byte"], byte, init0["min-byte"])
                and init0
              )
          end
          init = ret
        ...
        if
          code
          and (function(_77_, _78_, _79_)
            return (_77_ >= _78_) and (_78_ >= _79_)
          end)(init["max-code"], code, init["min-code"])
          and not (function(_80_, _81_, _82_)
            return (_80_ >= _81_) and (_81_ >= _82_)
          end)(57343, code, 55296)
        then
          return init.len
        else
          return nil
    ...
    local function pp_string(str, options, indent)
      ...
      local _86_
      if
        options["escape-newlines?"]
        and (length_2a(str) < (options["line-length"] - indent))
      then
        _86_ = "\\n"
      else
        _86_ = "\n"
...
package.preload["fennel.utils"] = package.preload["fennel.utils"]
  or function(...)
    local view = require("fennel.view")
    local version = "1.0.0"
    local function warn(message)
      if _G.io and _G.io.stderr then
        return (_G.io.stderr):write(
    ...
    local function list__3estring(self, _3ftostring2)
      ...
      return (
          "("
          .. table.concat(map(safe, (_3ftostring2 or view)), " ", 1, max)
          .. ")"
        )
    end
    ...
    local list_mt = {
      __fennelview = list__3estring,
      __tostring = list__3estring,
      "LIST",
    }
    local comment_mt = {
      __fennelview = comment_view,
      __tostring = deref,
      __eq = sym_3d,
      __lt = sym_3c,
      "COMMENT",
    ...
    local vararg = setmetatable(
      { "..." },
      { __fennelview = deref, __tostring = deref, "VARARG" }
    )
    local getenv
    ...
    local function comment_2a(contents, _3fsource)
      ...
      return setmetatable(
        { filename = filename, line = line, contents },
        comment_mt
      )
    end
    local function varg()
    ...
    local function table_3f(x)
      return (
          (type(x) == "table")
          and (x ~= vararg)
          and (getmetatable(x) ~= list_mt)
          and (getmetatable(x) ~= symbol_mt)
          and not comment_3f(x)
          and x
        )
    end
    ...
    root["set-reset"] = function(_130_)
      ...
      root.reset = function()
        root.chunk, root.scope, root.options, root.reset =
          chunk, scope, options, reset
        return nil
      end
      return root.reset
    end
    ...
    local function check_plugin_version(_132_)
      ...
      if
        not member_3f(version:gsub("-dev", ""), (versions or {}))
        and not warned[plugin]
      then
        warned[plugin] = true
        return warn(
          string.format(
            "plugin %s does not support Fennel version %s",
            (name or "unknown"),
            version
          )
        )
    ...
    return {
      ...
      ["ast-source"] = ast_source,
      version = version,
      path = table.concat(
        { "./?.fnl", "./?/init.fnl", getenv("FENNEL_PATH") },
        ";"
      ),
...
local function eval_env(env, opts)
  if env == "_COMPILER" then
    local env0 = specials["make-compiler-env"](
      nil,
...
local function syntax()
  ...
  for k, v in pairs(compiler.scopes.global.specials) do
    ...
    (out)[k] = {
      ["special?"] = true,
      ["body-form?"] = metadata["fnl/body-form?"],
      ["binding-form?"] = utils["member?"](k, binding_3f),
      ["define?"] = utils["member?"](k, define_3f),
    }
  end
  for k, v in pairs(compiler.scopes.global.macros) do
    out[k] = {
      ["macro?"] = true,
      ["body-form?"] = utils["member?"](k, body_3f),
      ["binding-form?"] = utils["member?"](k, binding_3f),
      ["define?"] = utils["member?"](k, define_3f),
    }
  end
...
do
  ...
  local env
  do
    local _621_ = specials["make-compiler-env"](
      nil,
      compiler.scopes.compiler,
      {}
    )
  ...
  local built_ins = eval(builtin_macros, {
    env = env,
  ...
  compiler.scopes.global.macros["\206\187"] =
    compiler.scopes.global.macros.lambda
